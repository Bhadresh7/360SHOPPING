...
export function AppLayout() {
    ...
    () => [
      ...
      },
      {
        ...
        items: [
          ...
          { icon: "💰", label: "Sell on Shopie", route: "view-seller-dashboard" }
        ]
      },
      {
  ...
  useEffect(() => {
    ...
    const updateParallax = () => {
      ...
      main.querySelectorAll<HTMLElement>("[data-parallax]").forEach((element) => {
  ...
  useEffect(() => {
    const setupMagnetic = (selector: string, scale = 1.04) => {
  ...
  return (
    <div className="app-shell">
      ...
      <nav className="mobile-nav">
        <button type="button" onClick={() => navigate("/view-dashboard")}>🏠</button>
        <button type="button" onClick={() => navigate("/view-studio")}>📸</button>
        <button type="button" onClick={() => navigate("/view-shop")}>🛍</button>
        <button type="button" onClick={() => navigate("/view-catalogue")}>🎁</button>
        <button type="button" onClick={() => navigate("/view-ai")}>🤖</button>
      </nav>
